...
class GAN():
    ...
    def train(self, epochs, train_data, batch_size):
        ...
        for epoch in range(epochs):
            #  Train Discriminator
            batch_indexes = np.random.randint(0, train_data.shape[0],
                                              batch_size)
            ...
            loss_fake = self.discriminator_model.train_on_batch(genenerated,
                                                                fake)
            discriminator_loss = 0.5 * np.add(loss_real, loss_fake)

            #  Train Generator
            noise = np.random.normal(0, 1,
                                     (batch_size, self.generator_input_dim))
    ...
    def _build_generator_model(self):
        generator_input = tf.keras.layers.Input(
            shape=(self.generator_input_dim, ))
        generator_seqence = tf.keras.models.Sequential(
            [tf.keras.layers.Dense(256,
                                   input_dim=self.generator_input_dim),
             tf.keras.layers.LeakyReLU(alpha=0.2),
             tf.keras.layers.BatchNormalization(momentum=0.8),
             tf.keras.layers.Dense(512), tf.keras.layers.LeakyReLU(alpha=0.2),
             tf.keras.layers.BatchNormalization(momentum=0.8),
             tf.keras.layers.Dense(1024), tf.keras.layers.LeakyReLU(alpha=0.2),
             tf.keras.layers.BatchNormalization(momentum=0.8),
             tf.keras.layers.Dense(np.prod(self.img_shape),
                                   activation='tanh'),
             tf.keras.layers.Reshape(self.img_shape)])

        generator_output_tensor = generator_seqence(generator_input)
        self.generator_model = tf.keras.models.Model(generator_input,
                                                     generator_output_tensor)

    def _build_and_compile_discriminator_model(self, optimizer):
        discriminator_input = tf.keras.layers.Input(shape=self.img_shape)
        discriminator_sequence = tf.keras.models.Sequential(
            [tf.keras.layers.Flatten(input_shape=self.img_shape),
             tf.keras.layers.Dense(512), tf.keras.layers.LeakyReLU(alpha=0.2),
             tf.keras.layers.Dense(256), tf.keras.layers.LeakyReLU(alpha=0.2),
             tf.keras.Dense(1,
                            activation='sigmoid')])

        discriminator_tensor = discriminator_sequence(discriminator_input)
        self.discriminator_model = tf.keras.models.Model(discriminator_input,
    ...
    def _build_and_compile_gan(self, optimizer):
        real_input = tf.keras.layers.Input(shape=(self.generator_input_dim, ))
        generator_output = self.generator_model(real_input)
        discriminator_output = self.discriminator_model(generator_output)

        self.gan = tf.keras.models.Model(real_input, discriminator_output)
